...
def down_sample(input_layer, n_filters, name,
    ...
    with tf.variable_scope("layer{}".format(name)):
        for i, F in enumerate(n_filters):
            ...
            if batch_norm:
                net = tf.keras.layers.BatchNormalization(name="bn_{}".format(i + 1))(net)
...
def up_sample(input_a, input_b, name, deconv=False):
    ...
    with tf.variable_scope("layer{}".format(name)):
        if deconv:
            net = tf.keras.layers.Conv3DTranspose(shape[-1].value, (3, 3, 3), strides=(2, 2, 2),
...
def max_pool_3d_with_argmax(input, kernel=(2,2,2), strides=(2,2,2)):
    """
    TODO
    :param input:
    :param kernel:
    ...
    :return:
    """
    pooled, switch = None, None
    return pooled, switch


def simple_unet_3d(input_shape, n_cls=2, batch_norm=False, deconv=False):
    ...
    with tf.variable_scope('down_sample'):
        l1, pool1 = down_sample(x, [32, 64], 'l1', batch_norm=batch_norm)
        l2, pool2 = down_sample(pool1, [64, 128], 'l2', batch_norm=batch_norm)
        l3, pool3 = down_sample(pool2, [128, 256], 'l3', batch_norm=batch_norm)
