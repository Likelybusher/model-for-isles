...
from ..metrics import cross_entropy_loss_with_weight, dice_coef, cross_entropy_loss
...
def train_process(config, debug=False):
    if debug:
    ...
    with tf.Session(graph=tf.get_default_graph()) as sess:
        ...
        save_path_prefix = os.path.join(train_logdir, "ckpt")
        # save the result
        for epoch in range(config["n_epochs"]):
            ...
            if not epoch % 50:
                saver.save(sess, save_path_prefix)
